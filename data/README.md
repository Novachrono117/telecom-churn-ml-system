...
This directory holds the project's data layers. **The raw source file is
versioned**: it is small (<1 MB), immutable, and is the single input every
recorded result is traceable to, so a fresh clone reproduces the pipeline
without a Kaggle account. Derived layers are **not** versioned — they are
regenerated by code (see `.gitignore`).

```text
data/
├── README.md      <- tracked
├── raw/           <- tracked; untouched source file, never edited by hand
├── interim/       <- not tracked; rebuilt by code
└── processed/     <- not tracked; rebuilt by code
```

The acquisition steps below remain the reference for replacing the file or
moving to a new dataset version; they are no longer a prerequisite for cloning.
